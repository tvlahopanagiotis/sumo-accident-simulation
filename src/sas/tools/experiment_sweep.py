...
For each (period, prob, seed) cell the script:
  ...
  3. Runs the packaged SAS runner as a subprocess
  4. Reads the output JSONs and network_metrics.csv
  5. Appends one row to  results/sweep/sweep_results.csv

Usage
-----
    sas-sweep                          # full grid (~70 runs)
    sas-sweep --seeds 3                # triplicate every cell
    sas-sweep --periods 2.0 1.0 0.5    # subset of periods
    sas-sweep --probs 0 1.5e-4 1e-3    # subset of probs
...
def main():
    ...
    print(f"  Runner   : python -m sas.simulation.runner")
