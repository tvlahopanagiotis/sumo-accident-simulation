...
Default input:
  data/cities/seattle/bundle/traffic_dataset/02_Seattle/01_Input_data/Seattle.osm

Outputs (default out-dir: ./data/cities/seattle/network):
  seattle.osm
  seattle.net.xml
  seattle.rou.xml
  seattle.sumocfg
...
DEFAULT_OUT_DIR = os.path.join(ROOT_DIR, "data", "cities", "seattle", "network")
...
def main() -> None:
    ...
    ap.add_argument(
        ...
        help="Output directory (default: ./data/cities/seattle/network)",
    )
    ap.add_argument(
        "--period",
        type=float,
    ...
    ap.add_argument(
        "--config",
        default=str(CONFIGS_DIR / "seattle" / "default.yaml"),
    ...
    if not os.path.exists(args.osm_file):
        ...
        print(
            '    suma-fetch-osm --place "Seattle, Washington, USA" '
            f'--out "{args.osm_file}"'
        )
        sys.exit(1)
