...
Usage
-----
    suma-generate-sioux-falls                  # generate network + routes
    suma-generate-sioux-falls --update-config  # also patch the selected config file
    suma-generate-sioux-falls --period 2.0     # adjust vehicle density
...
from ..app.config import DEFAULT_CONFIG_PATH, PROJECT_ROOT
...
def write_nodes(path: str):
    ...
    for nid, (x, y) in NODES.items():
        ET.SubElement(root, "node", id=str(nid), x=str(x), y=str(y), type="priority")
...
def write_edges(path: str):
    ...
    with open(path, "w") as f:
        f.write(_pretty_xml(root))
    print(
        f"    edges.xml  — {len(LINKS) * 2} directed edges  ({total_km:.1f} km total road length)"
    )
...
def main():
    ap = argparse.ArgumentParser(description="Generate the Sioux Falls benchmark network for SUMO")
    ap.add_argument(
        "--out-dir",
        default=str(PROJECT_ROOT / "data" / "benchmarks" / "sioux_falls" / "network"),
        help="Output directory (default: ./data/benchmarks/sioux_falls/network/)",
    )
    ap.add_argument(
        "--period",
        type=float,
    ...
    ap.add_argument(
        "--update-config",
        action="store_true",
        help="Patch the selected YAML config so SUMA uses this network immediately",
    )
    ap.add_argument(
        "--config",
        default=str(DEFAULT_CONFIG_PATH),
        help=f"Path to YAML config to update (default: {DEFAULT_CONFIG_PATH})",
    )
    args = ap.parse_args()
    ...
    print(f"\n{'=' * 55}")
    print("  Sioux Falls benchmark network  (LeBlanc et al. 1975)")
    print("  24 nodes · 76 directed links · 50 km/h arterial grid")
    print(f"{'=' * 55}\n")
    ...
    if rand_trips is None:
        ...
        print(
            f"      randomTrips.py -n {net_path} -o {rou_path} "
    ...
    write_sumocfg(cfg_path, os.path.basename(net_path), os.path.basename(rou_path))

    # 5. Optionally patch the selected config file
    if args.update_config and os.path.exists(args.config):
        ...
        print(f"\n  config updated → sumo.config_file = {cfg_path}")
