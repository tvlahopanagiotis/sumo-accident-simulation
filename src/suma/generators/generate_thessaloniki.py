...
This script:
  ...
  5. Optionally patches a YAML config so SUMA uses this network immediately
...
Usage:
  suma-generate-thessaloniki                  # build network + routes
  suma-generate-thessaloniki --update-config  # also patch the selected config file
  suma-generate-thessaloniki --period 2.0     # sparser traffic (fewer vehicles)
  suma-generate-thessaloniki --gui            # open sumo-gui after building
...
# ---------------------------------------------------------------------------
# Step 5 — Patch the selected config file
# ---------------------------------------------------------------------------
...
def main() -> None:
    ...
    ap.add_argument(
        "--out-dir",
        default=str(PROJECT_ROOT / "data" / "cities" / "thessaloniki" / "network"),
        help="Output directory (default: ./data/cities/thessaloniki/network/)",
    )
    ap.add_argument(
        "--period",
        type=float,
    ...
    ap.add_argument(
        "--gui",
        action="store_true",
        help="Set binary to sumo-gui in the selected config file",
    )
    ap.add_argument(
        "--skip-download",
        action="store_true",
    ...
    # 5. Optionally update the selected config file
    ...
    print("  Network ready!")
    print("\n  Next steps:")
    print(f"    1. Update config → sumo.config_file: {cfg_path}")
