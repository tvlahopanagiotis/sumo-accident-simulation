...
Output folder (--out-dir, default: data/synthetic/riverside/network/):
...
Usage:
  suma-generate-riverside                         # build in ./data/synthetic/riverside/network/
  suma-generate-riverside --out-dir /tmp/mynet    # custom output folder
  suma-generate-riverside --update-config         # also patch the selected config file
"""
...
def update_config_yaml(config_path: str, sumocfg_path: str):
    """Patch the binary and config_file keys in a YAML config file."""
    ...
    print("\n✅  config updated → config_file now points to the new network.")
...
def main():
    ...
    parser.add_argument(
        "--out-dir",
        default=str(PROJECT_ROOT / "data" / "synthetic" / "riverside" / "network"),
        help="Output folder for network files (default: ./data/synthetic/riverside/network/)",
    )
    parser.add_argument(
        "--update-config",
        action="store_true",
        help="Also update the selected config file to point at the new network",
    )
    ...
    # Optionally patch the selected config file
    if args.update_config:
        config_yaml = str(DEFAULT_CONFIG_PATH)
        if os.path.exists(config_yaml):
            update_config_yaml(config_yaml, cfg_file)
        else:
            print(f"⚠  config file not found at {config_yaml} — skipping update.")

    print(f"\n{'=' * 60}")
    print("  Done!  Run the simulation with:")
    print("    suma")
    print(f"  (after updating config → sumo.config_file: {cfg_file})")
    print(f"{'=' * 60}\n")
