...
For each (period, prob, seed) cell the script:
  ...
  3. Runs the packaged SUMA runner as a subprocess
  4. Reads the output JSONs and network_metrics.csv
  5. Appends one row to  results/sweep/sweep_results.csv

Usage
-----
    suma-sweep                          # full grid (~70 runs)
    suma-sweep --seeds 3                # triplicate every cell
    suma-sweep --periods 2.0 1.0 0.5    # subset of periods
    suma-sweep --probs 0 1.5e-4 1e-3    # subset of probs
...
def main():
    ap = argparse.ArgumentParser(
        description="SUMA failure-point parameter grid sweep",
        formatter_class=argparse.ArgumentDefaultsHelpFormatter,
    )
    ap.add_argument(
        "--config",
        default=os.path.join(REPO_ROOT, "configs", "thessaloniki", "default.yaml"),
    ...
    ap.add_argument(
        "--out-dir",
        default=os.path.join(REPO_ROOT, "results", "sweep"),
    ...
    print(f"\n  SUMA — Failure-Point Sweep")
    print(f"  {'─'*40}")
    print(f"  Config   : {args.config}")
    print(f"  Runner   : python -m suma.simulation.runner")
