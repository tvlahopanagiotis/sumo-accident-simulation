...
if TYPE_CHECKING:
    from .accident_manager import Accident

# Short alias for TraCI constants
_tc = traci.constants

logger = logging.getLogger("suma.metrics")
...
class MetricsCollector:
    ...
    def __init__(self, config: dict[str, Any], output_config: dict[str, Any]) -> None:
        """
        Args:
    ...
    def record_accident_resolved(self, accident: Accident) -> None:
        """
        ...
        logger.info(
            "Accident %s recorded (duration %ds).",
    ...
    def compute_antifragility_index(self) -> dict[str, object]:
        ...
        if mean_ai > 0.05:
            result["interpretation"] = (
                "ANTIFRAGILE — Network performance improved after disruptions"
            )
        elif mean_ai > -0.05:
    ...
    def _register_recovery_monitor(self, accident):
        ...
        if not pre_speeds:
            ...
            pre_speeds = [
                e["mean_speed"] for e in self._speed_buffer if pre_lo <= e["step"] < trigger
            ]
