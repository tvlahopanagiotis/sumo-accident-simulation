...
Entry point for the Simulator for Urban Mobility Antifragility.
...
Custom config:
    suma --config configs/thessaloniki/default.yaml --runs 5
...
logger = logging.getLogger("suma.runner")
...
def run_once(
    ...
    logger.info("=" * 60)
    logger.info("  SUMA — %d simulation seconds  (seed %d)", total_steps, run_seed)
    logger.info("=" * 60)
...
def main(argv: list[str] | None = None) -> None:
    parser = argparse.ArgumentParser(
        ...
        epilog=(
            "examples:\n"
            "  suma                                           # single run, default config\n"
            "  suma --runs 10                                 # 10 runs, aggregate statistics\n"
            "  suma --config configs/thessaloniki/default.yaml --runs 5\n"
            "  suma --log-level DEBUG                         # verbose output\n"
            "\n"
